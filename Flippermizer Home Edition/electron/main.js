...
const TASK_REPOSITORY_USER_CFG_FILE = "flippermizer-task-repository-cfg.json";
...
function getTaskRepositoryCfgPath(){
  return path.join(getAppRootDir(), TASK_REPOSITORY_CFG_FILE);
}

function getTaskRepositoryUserCfgPath(){
  return path.join(getUserDataDir(), TASK_REPOSITORY_USER_CFG_FILE);
}
...
function installTaskRepositoryIpc(){
  ...
  ipcMain.handle("flpr-task-repository-cfg:save", (_event, payload)=>{
    return writeTaskRepositoryCfg(payload || {});
  });
}

function installDevToolsIpc(){
  ipcMain.handle("flpr-dev-tools:unlock", (_event, password)=>{
    return { ok: verifyDevToolsPassword(password) };
  });
}
