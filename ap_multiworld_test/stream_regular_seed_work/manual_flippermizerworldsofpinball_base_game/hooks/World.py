...
from ..Helpers import is_option_enabled, get_option_value, is_item_enabled, format_state_prog_items_key, ProgItemsCat, load_data_file
...
def _get_enabled_trap_item_names(world: World, multiworld: MultiWorld, player: int) -> list[str]:
    enabled_traps: list[str] = []
    for item_name, item_def in getattr(world, "item_name_to_item", {}).items():
        if not isinstance(item_def, dict) or not item_def.get("trap"):
            continue
        try:
            if "category" in item_def and not is_item_enabled(multiworld, player, item_def):
                continue
        except Exception:
            logging.exception("Base Game: failed to evaluate trap item option for %s.", item_name)
            continue
        enabled_traps.append(item_name)
    return enabled_traps


def _apply_stream_filler_traps(item_pool: list, world: World, multiworld: MultiWorld, player: int) -> list:
    try:
        trap_percent = int(get_option_value(multiworld, player, "filler_traps") or 0)
    except Exception:
        trap_percent = 0
    trap_percent = max(0, min(100, trap_percent))
    if trap_percent <= 0:
        return item_pool

    trap_names = _get_enabled_trap_item_names(world, multiworld, player)
    if not trap_names:
        return item_pool

    filler_items = [item for item in item_pool if getattr(item, "classification", None) == ItemClassification.filler]
    trap_count = len(filler_items) * trap_percent // 100
    if trap_count <= 0:
        return item_pool

    world.random.shuffle(filler_items)
    for filler_item in filler_items[:trap_count]:
        try:
            item_pool.remove(filler_item)
        except ValueError:
            continue
        item_pool.append(world.create_item(world.random.choice(trap_names)))

    logging.info(
        "Base Game: replaced %d filler items with traps using filler_traps=%d%%.",
        trap_count,
        trap_percent,
    )
    return item_pool
...
def before_create_items_filler(item_pool: list, world: World, multiworld: MultiWorld, player: int) -> list:
    item_pool = _apply_stream_filler_traps(item_pool, world, multiworld, player)
