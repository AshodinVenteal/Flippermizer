...
class ManualWorld(World):
    ...
    def create_items(self):
        ...
        effective_starting_items = starting_items
        try:
            if get_option_value(self.multiworld, self.player, "random_start_tables_open"):
                effective_starting_items = []
        except Exception:
            pass
        metasizer_starting_override = getattr(self, "_metasizer_starting_items_override", None)
        if isinstance(metasizer_starting_override, list):
            effective_starting_items = metasizer_starting_override

        if effective_starting_items:
            for starting_item_block in effective_starting_items:
    ...
    def generate_basic(self):
        ...
        for location in locations_with_placements:
            ...
            # Remove the exact placed object from the pool. Boss Key has duplicates,
            # and list.remove can match an equal sibling instead of this instance.
            for pool_index, pool_item in enumerate(self.multiworld.itempool):
                if pool_item is item_to_place:
                    del self.multiworld.itempool[pool_index]
                    break
